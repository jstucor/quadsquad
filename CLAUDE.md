...
- **EVERYTHING THAT IS PUT SOMEWHERE ASKS ONE FUNCTION WHERE THE GROUND IS**
  (`GameState.ground_at`, with `standable()` beside it). The capture area, the command posts and
  the royale crates each cast their OWN ray straight down from 80 or 120 m against the world mask
  — right on open ground, and on a ROOFED map it is the ceiling. Measured on the Outpost: the
  capture area sat 7.5 m up, on the roof, for the whole match. Nothing errors — Zones simply never
  scores, Conquest never bleeds and the loot is where nobody can reach it, so all three read as
  balance problems. `ground_at` excludes the bodies a map has already flagged `MAP_NAV_IGNORE`,
  which is the same fact the nav grid reads, from the other direction.
- **AND "ON THE FLOOR" IS NOT ENOUGH — IT HAS TO BE SOMEWHERE A BODY CAN STAND.** On open ground
  with cover scattered over it there is nowhere unreachable to land; on a base of rooms a third of
  the map is wall, and a capture point inside one is a mode that cannot be played. `standable()`
  asks the nav grid, and Conquest additionally SPIRALS OUT from the post it wanted
  (`_standable_near`) rather than trusting its line down the middle — on an open map that returns
  the original point on the first try and costs nothing.
