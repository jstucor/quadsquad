...
## The Compact Wars five come first, so nothing that already names Kit.LEGION moved.
enum Kit {
	LEGION, HUNTER, ADEPT, URSAN, SAURIAN,
	PALADIN, DROPTROOPER, ZHAAL, KOPA, URSID,          # Deep Range
	SENTINEL, CHORISTER, UNSLEEPING, SCRAPKIN,                 # Ironhymn
}
...
##
## Every catalogue row states the universe it belongs to. NO KEY MEANS THE COMPACT WARS,
...
enum Universe { COMPACT, DEEP_RANGE, IRONHYMN }
const ANY_UNIVERSE := -1

const UNIVERSES: Array[Dictionary] = [
	{
		"name": "THE COMPACT WARS",
		"blurb": "Legionaries, automata, adepts and hunters",
		"kit": Kit.LEGION,          # what a class-free build (royale, starter) wears
		# FOUR ERAS ON ONE FIELD, which is the point of a setting rather than a
		# campaign: the Compact Wars pair and the Galactic Civil War pair, and any
		# of the four can be fielded against any other.
		"teams": ["CONCORD LEGION", "AUTOMATA", "IRON DOMINION", "EMBER PACT"],
		"colors": [Color(0.35, 0.55, 1.0), Color(1.0, 0.40, 0.32),
			Color(0.62, 0.66, 0.72), Color(0.95, 0.62, 0.22)],
		# Legion blue against droid red is the Compact Wars, and Dominion green
	...
	{
		"name": "DEEP RANGE",
		"blurb": "Coalition ballistics against Hierophant plasma",
		"kit": Kit.PALADIN,
		"teams": ["COALITION", "HIEROPHANY", "THE SUNDERED", "PRECURSORS"],
		"colors": [Color(0.38, 0.78, 0.42), Color(0.70, 0.45, 1.0),
			Color(1.0, 0.45, 0.22), Color(0.45, 0.85, 0.95)],
		# Brass tracer against plasma. Most Hierophany guns state their own colour
	...
	{
		"name": "IRONHYMN",
		"blurb": "Two crusader orders, the deathless and the scrapkin",
		"kit": Kit.SENTINEL,
		"teams": ["AZURE SENTINELS", "CRIMSON CHOIR", "THE UNSLEEPING", "SCRAPKIN"],
		...
		# mass-reactive shell, and a shellgun's tracer is not a chapter badge.
...
## --- FACTIONS, FLAT --------------------------------------------------------
##
## A SIDE IS NOW PICKED INDEPENDENTLY OF THE SETTING, so COALITION can fight the
## Concord. `UNIVERSES` still groups the catalogue — which classes and guns a
...
## roster it fields.
##
## Deep Range lists four team names but authors only two rosters, so `faction_count`
	...
	for u in UNIVERSES.size():
		...
		for side in rosters.size():
			_factions.append({
				...
				# "THE UNSLEEPING" alone does not say which game you are looking at.
...
## The first faction of a universe, so the menu can offer "all The Compact Wars" as a
...
## map, and the note on `bolts` is that the Dominion's grey plate would make a grey
...
## ask.
static var active_universe := Universe.COMPACT
...
## already been handed. Measured before this: The Compact Wars reached 30 of 76
## primaries and Deep Range 17, with the Ursan able to hold TWO and the Kopa three.
##
## What it does NOT relax is `"kit"` exclusivity. A saber, a quarrel caster, a thermal
## holo belong to their owner in every mode, because those are MECHANISM — a
## saber needs the guard and a Kinesis power needs the button — where a per-kit
## list is only balance. That is the same line `royale_items` already draws.
static var custom_pool := false


## Which universe a catalogue row belongs to. Missing means The Compact Wars: the whole
## original catalogue predates the setting and says so by saying nothing.
static func entry_universe(entry: Dictionary) -> int:
	return int(entry.get("universe", Universe.COMPACT))
...
static func kit_universe(of_kit: int) -> int:
	return int(KITS[clampi(of_kit, 0, KITS.size() - 1)].get("universe", Universe.COMPACT))
...
##   secondary_mods which SecondaryMod ids it may buy — DUAL is Hunter-only
...
##   universe       which setting the class plays in (absent = The Compact Wars)
...
## without an explicit `primaries` list still cannot pick up a arc blade. A gun
...
const WEAPONS: Array[Dictionary] = [
	...
	# The two heaviest things a body can carry are the URSAN's alone. They keep
	# a `royale` flag because that mode has no classes at all: a rocket tube on
	# the ground works perfectly well for a plain trooper, unlike a arc blade
	# with no guard behind it, so kit-locking them in the shop must not quietly
	# empty two of the best finds out of the crates.
	{"class": Weapon.Class.HMG, "cost": 80, "kit": Kit.URSAN, "royale": true},
	{"class": Weapon.Class.SNIPER, "cost": 85},
	{"class": Weapon.Class.RPG, "cost": 110, "kit": Kit.URSAN, "royale": true},
	...
	{"class": Weapon.Class.SABER, "cost": 55, "kit": Kit.ADEPT},

	# --- HALO ----------------------------------------------------------------
	# Priced against the The Compact Wars column above rather than against each other,
	# so a 200-token budget buys the same SHAPE of build in any universe: a rifle
	# and a gadget, or a heavy gun and nothing else.
	{"class": Weapon.Class.S9_SMG, "cost": 40, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.AR7, "cost": 45, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.PLASMA_RIFLE, "cost": 45, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.BR3, "cost": 55, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.HIER_CARBINE, "cost": 55, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.NEEDLER, "cost": 55, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.CQ12, "cost": 60, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.ENERGY_SWORD, "cost": 60, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.GRAV_HAMMER, "cost": 70, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.CLEAVER_GUN, "cost": 70, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.DM3, "cost": 75, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.HM40, "cost": 80, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.LR99, "cost": 85, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.BEAM_RIFLE, "cost": 85, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.FUEL_ROD, "cost": 105, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.RL8, "cost": 110, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.LANCE_LASER, "cost": 115, "universe": Universe.DEEP_RANGE},

	# --- IRONHYMN ----------------------------------------------------
	{"class": Weapon.Class.CLEAVER, "cost": 40, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.SLUGTHROWER, "cost": 45, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.GAUSS_RIFLE, "cost": 45, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.CHAINSWORD, "cost": 45, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.SHELLGUN, "cost": 50, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.TESLA_CARBINE, "cost": 50, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.STAFF_OF_LIGHT, "cost": 55, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.TORCH, "cost": 55, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.FLAMER, "cost": 60, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.GAUSS_BLASTER, "cost": 60, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.POWER_SWORD, "cost": 60, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.HEAT_RAY, "cost": 65, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.WARSCYTHE, "cost": 65, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.CRUSHER_CLAW, "cost": 70, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.BIG_SHOOTA, "cost": 70, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.THUNDER_HAMMER, "cost": 75, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.HEAVY_BOLTER, "cost": 80, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.STALKER_BOLT, "cost": 80, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.JUNK_BLASTER, "cost": 80, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.FUSION_GUN, "cost": 85, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.SYNAPTIC_DISINTEGRATOR, "cost": 85, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.PLASMA_GUN, "cost": 90, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.GRENADE_LAUNCHER, "cost": 95, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.SCRAP_ROCKET, "cost": 105, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.TRANSDIMENSIONAL_BEAMER, "cost": 110, "universe": Universe.IRONHYMN},
	# --- THE FACTION GUNS -----------------------------------------------------
	...
	{"class": Weapon.Class.VL15S, "cost": 45},
	{"class": Weapon.Class.VL17M, "cost": 60},
	{"class": Weapon.Class.VL15X, "cost": 90},
	{"class": Weapon.Class.X5, "cost": 35},
	{"class": Weapon.Class.X5S, "cost": 85},
	{"class": Weapon.Class.AEGIS_TWIN, "cost": 85},
	{"class": Weapon.Class.SONIC_BLASTER, "cost": 70},
	{"class": Weapon.Class.DK11, "cost": 45},
	{"class": Weapon.Class.DK19, "cost": 75},
	{"class": Weapon.Class.DK20A, "cost": 80},
	{"class": Weapon.Class.FLAMETHROWER, "cost": 65},
	{"class": Weapon.Class.A28C, "cost": 55},
	{"class": Weapon.Class.CR9, "cost": 45},
	{"class": Weapon.Class.DH44, "cost": 90},
	{"class": Weapon.Class.GL19, "cost": 75, "universe": Universe.DEEP_RANGE},
	...
	{"class": Weapon.Class.DK11D, "cost": 60},
	{"class": Weapon.Class.SPIKER, "cost": 50, "universe": Universe.DEEP_RANGE},
	...
	# rocket tube are. Those two are the Ursan's because they are the heaviest
	...
	{"class": Weapon.Class.SAW7, "cost": 70, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.DK19D, "cost": 75},
	{"class": Weapon.Class.RT9, "cost": 85},
	{"class": Weapon.Class.GAUSS_CANNON, "cost": 85, "universe": Universe.IRONHYMN},
]
...
const SECONDARIES: Array[Dictionary] = [
	...
	{"class": Weapon.Class.DH7, "cost": 15},
	{"class": Weapon.Class.HOLDOUT, "cost": 20},
	{"class": Weapon.Class.REVOLVER, "cost": 35},
	{"class": Weapon.Class.BR9, "cost": 40},
	# The Ursan's, and the only sidearm it may carry. Free for the same reason
	...
	{"class": Weapon.Class.QUARREL_CASTER, "cost": 0, "kit": Kit.URSAN},
	...
	{"class": Weapon.Class.S6, "cost": 0, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.PLASMA_PISTOL, "cost": 0, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.MAULER, "cost": 30, "universe": Universe.DEEP_RANGE},
	{"class": Weapon.Class.BOLT_PISTOL, "cost": 0, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.GAUSS_PISTOL, "cost": 0, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.SLUG_PISTOL, "cost": 0, "universe": Universe.IRONHYMN},
	{"class": Weapon.Class.ORDER_PLASMA_PISTOL, "cost": 30, "universe": Universe.IRONHYMN},
	...
	{"class": Weapon.Class.VL17, "cost": 30},
	{"class": Weapon.Class.SR14R, "cost": 35},
	{"class": Weapon.Class.SPIKER, "cost": 35, "universe": Universe.DEEP_RANGE},
]
...
const SIGHTS: Array[Dictionary] = [
	...
	# The Saurian's, and only theirs: a holo ring that paints enemy HEAT while
	...
	{"name": "THERMAL HOLO", "cost": 30, "kit": Kit.SAURIAN,
...
enum Gadget { NONE, JETPACK, CABLE, SHIELD, ROTARY, TURRET, MORTAR,
	KINETIC_PUSH, KINETIC_PULL, KINETIC_LEAP, ARC_STORM, WRIST_ROCKET,
	...
	# Ironhymn — Order
	JUMP_PACK, IRON_HALO, AUGUR_SCAN, ORBITAL_BOMBARDMENT, ASSAULT_CANNON,
	CRIMSON_RAGE, BREACH_CHARGE, FUSION_CHARGE,
	# ...Necrons
	TESLA_ARC, PHASE_SHIFT, TRANSLOCATION, SENTRY_CONSTRUCT,
	# ...Orks
	WARCRY, RUNT_GUNNER, KUSTOM_FORCE_FIELD, SCRAP_JETS, SCRAP_BOMB,
	SMOKE_LAUNCHER,
	# --- SUSTAINED (slot 3) ---------------------------------------------------
	...
	# of Vespid wings is a jetpack. Two are genuinely new mechanisms —
...
const GADGETS: Array[Dictionary] = [
	...
	# The barrier is the URSAN's now. Same `royale` opt-in as the heavy guns:
	# class-locked in the shop, still findable on the ground in a mode that has
	# no classes to lock it to.
	{"name": "FRONT SHIELD", "cost": 50, "kit": Kit.URSAN, "royale": true,
	...
	{"name": "ARC STORM", "cost": 50, "kit": Kit.ADEPT,
		"blurb": "HOLD to pour lightning at the nearest enemy in front, chaining to 3 more. 18 m, 4s"},
	# The Hunter's third gadget, and the only DAMAGE one it can buy. No
	...
	# The Saurian's two. No "kit" key — the allow-list keeps them theirs, and
	...
	# grenade type (see Player._use_gadget). SMOKE is the Saurian's alone, so
	...
	{"name": "SMOKE GRENADE", "cost": 20, "kit": Kit.SAURIAN,
		"blurb": "Lob smoke: blinds the area, nothing sees through it (you do, thermal). 7s"},
	# The Legion ARC's recon tool: fire a dart that sticks and PINGS nearby enemies
	...
	# --- HALO ----------------------------------------------------------------
	{"name": "BUBBLE SHIELD", "cost": 50, "universe": Universe.DEEP_RANGE, "like": Gadget.SHIELD,
		"blurb": "Toggle a barrier that stops incoming fire. Shoot through it, but no aiming"},
	{"name": "ACTIVE CAMO", "cost": 45, "universe": Universe.DEEP_RANGE, "like": Gadget.CLOAK,
		"blurb": "Vanish for a few seconds. Firing drops it. AI cannot see you cloaked. 9s"},
	{"name": "THRUSTER PACK", "cost": 25, "universe": Universe.DEEP_RANGE, "like": Gadget.DASH,
		"blurb": "A quick burst in the way you are moving. 4s"},
	{"name": "AUTOSENTRY", "cost": 65, "universe": Universe.DEEP_RANGE, "like": Gadget.TURRET,
		"blurb": "Drop an auto-turret that fights for you until it's destroyed"},
	{"name": "GRAV LIFT", "cost": 35, "universe": Universe.DEEP_RANGE, "like": Gadget.KINETIC_LEAP,
		"blurb": "A hard shove upward and forward: high, long, and it closes ground fast. 4s"},
	{"name": "JET PACK", "cost": 45, "universe": Universe.DEEP_RANGE, "like": Gadget.JETPACK,
		"blurb": "Hold the gadget button to fly. Fuel burns fast, refills on the ground"},
	{"name": "PLASMA CANNON", "cost": 75, "universe": Universe.DEEP_RANGE, "like": Gadget.ROTARY,
		"blurb": "Tear a plasma cannon off its mount and carry it. Huge output, but you walk"},
	{"name": "TARGET DESIGNATOR", "cost": 60, "universe": Universe.DEEP_RANGE, "like": Gadget.MORTAR,
		"blurb": "Drop a tube, then call salvos from the map screen. 14s between them"},
	{"name": "TRACKER DART", "cost": 30, "universe": Universe.DEEP_RANGE, "like": Gadget.SCAN_DART,
		"blurb": "A dart that sticks and reveals enemies near it to your team through walls. 12s"},
	{"name": "FRAG CHARGE", "cost": 30, "universe": Universe.DEEP_RANGE, "like": Gadget.GRENADE_FRAG,
		"blurb": "Lob a frag: bounces, 2s fuse, heavy splash. 6s between throws"},
	...
	{"name": "PLASMA GRENADE", "cost": 35, "universe": Universe.DEEP_RANGE, "like": Gadget.GRENADE_STICKY,
		"blurb": "Lob a plasma stick: clings where it lands, people included, then blows. 6s"},

	# --- IRONHYMN: Adeptus Order ----------------------------------
	{"name": "JUMP PACK", "cost": 45, "universe": Universe.IRONHYMN, "like": Gadget.JETPACK,
		"blurb": "Hold the gadget button to fly. Fuel burns fast, refills on the ground"},
	{"name": "AEGIS HALO", "cost": 50, "universe": Universe.IRONHYMN, "like": Gadget.SHIELD,
		"blurb": "Toggle a barrier that stops incoming fire. Shoot through it, but no aiming"},
	{"name": "AUGUR SCAN", "cost": 30, "universe": Universe.IRONHYMN, "like": Gadget.SCAN_DART,
		"blurb": "A scanner that sticks and reveals enemies near it to your squad through walls. 12s"},
	{"name": "ORBITAL BOMBARDMENT", "cost": 60, "universe": Universe.IRONHYMN, "like": Gadget.MORTAR,
		"blurb": "Plant a beacon, then call salvos from the map screen. 14s between them"},
	{"name": "ASSAULT CANNON", "cost": 75, "universe": Universe.IRONHYMN, "like": Gadget.ROTARY,
		"blurb": "Toggle a spin-up rotary cannon. Huge output, but you walk"},
	{"name": "CRIMSON RAGE", "cost": 25, "universe": Universe.IRONHYMN, "like": Gadget.FURY,
		"blurb": "The thirst takes you: faster, tougher, and a heavier swing. 8s"},
	{"name": "BREACH CHARGE", "cost": 30, "universe": Universe.IRONHYMN, "like": Gadget.GRENADE_FRAG,
		"blurb": "Lob a krak: bounces, 2s fuse, heavy splash. 6s between throws"},
	{"name": "FUSION CHARGE", "cost": 35, "universe": Universe.IRONHYMN, "like": Gadget.GRENADE_STICKY,
		"blurb": "Lob a melta bomb: clings where it lands, people included, then blows. 6s"},
	# --- ...Necrons -----------------------------------------------------------
	{"name": "TESLA ARC", "cost": 50, "universe": Universe.IRONHYMN, "like": Gadget.ARC_STORM,
		"blurb": "HOLD to pour arc lightning at the nearest enemy in front, chaining to 3 more. 18 m, 4s"},
	{"name": "PHASE SHIFT", "cost": 45, "universe": Universe.IRONHYMN, "like": Gadget.CLOAK,
		"blurb": "Phase out for a few seconds. Firing drops it. AI cannot see you phased. 9s"},
	{"name": "TRANSLOCATION", "cost": 35, "universe": Universe.IRONHYMN, "like": Gadget.KINETIC_LEAP,
		"blurb": "A dimensional bound: high, long, and it closes ground fast. 4s"},
	{"name": "SENTRY CONSTRUCT", "cost": 65, "universe": Universe.IRONHYMN, "like": Gadget.TURRET,
		"blurb": "Drop a construct that fights for you until it's destroyed"},
	# --- ...Orks --------------------------------------------------------------
	{"name": "WARCRY", "cost": 25, "universe": Universe.IRONHYMN, "like": Gadget.FURY,
		"blurb": "Roar. Faster, harder to put down, and a heavier swing. 8s"},
	{"name": "RUNT GUNNER", "cost": 65, "universe": Universe.IRONHYMN, "like": Gadget.TURRET,
		"blurb": "Drop a grot on a gun that fights for you until it's destroyed"},
	{"name": "JUNK FIELD", "cost": 50, "universe": Universe.IRONHYMN, "like": Gadget.SHIELD,
		"blurb": "Toggle a barrier that stops incoming fire. Shoot through it, but no aiming"},
	{"name": "SCRAP JETS", "cost": 45, "universe": Universe.IRONHYMN, "like": Gadget.JETPACK,
		"blurb": "Hold the gadget button to fly. Fuel burns fast, refills on the ground"},
	{"name": "SCRAP BOMB", "cost": 25, "universe": Universe.IRONHYMN, "like": Gadget.GRENADE_FRAG,
		"blurb": "Lob a scrap bomb: bounces, 2s fuse, heavy splash. 6s between throws"},
	...
	# pulse, deployable cover was a set of Vespid wings, and BATTLE FURY was
	# a medkit. Nothing errored, because every one of them is a valid gadget.
	{"name": "SMOKE LAUNCHER", "cost": 20, "universe": Universe.IRONHYMN, "like": Gadget.GRENADE_SMOKE,
	...
	{"name": "DEPLOYABLE COVER", "cost": 50, "universe": ANY_UNIVERSE,
		"like": Gadget.SHIELD,
		"blurb": "Toggle a barrier that stops incoming fire. Shoot through it, but no aiming"},
	{"name": "VESPID WINGS", "cost": 40, "universe": Universe.COMPACT,
		"like": Gadget.JETPACK,
		"blurb": "Hold the gadget button to fly. Wingbeats burn fast, refill on the ground"},
	{"name": "HELMET SCAN", "cost": 30, "universe": Universe.DEEP_RANGE, "like": Gadget.SCAN_DART,
		"blurb": "Light everyone up for your fireteam, through walls"},
	{"name": "THERMAL CHARGE", "cost": 30, "universe": Universe.COMPACT,
...
const GADGET_COOLDOWNS := {
	Gadget.KINETIC_PUSH: 6.0, Gadget.KINETIC_PULL: 7.0, Gadget.KINETIC_LEAP: 4.0,
...
const KITS: Array[Dictionary] = [
	{
		"name": "LEGIONARY",
		"blurb": "Concord line trooper. Every rifle worth carrying and every deployable",
		"gadgets": [
		...
		"style": CharacterModel.Style.LEGION,
	},
	{
		"name": "HUNTER",
		...
		"style": CharacterModel.Style.HUNTER,
	},
	{
		"name": "ADEPT",
		"blurb": "Arc Blade or any gun, plus a Kinesis power. Tough, fast, dashes and double jumps",
		...
		# the other on a Kinesis power, rather than choosing between a power and the
		# dash that only appears when a slot is left empty.
		"gadgets": [
			Gadget.NONE, Gadget.KINETIC_PUSH, Gadget.KINETIC_PULL, Gadget.KINETIC_LEAP,
			Gadget.ARC_STORM, Gadget.DASH, Gadget.GRENADE_FRAG,
		...
		# is reached through its own "kit" key (see allows). Kinesis gadget on
		# the gadget button and the dash/double-jump are the class, not the blade,
		# so a gun-toting Warden keeps all of them; only the GUARD needs the saber
		...
		"health": 1.25,
		"dash": true,
		"style": CharacterModel.Style.WARDEN,
	},
	{
		"name": "URSAN",
		"blurb": "Heavy weapons and a quarrel caster. Slow, enormously tough, and the only one with a barrier",
		# One gadget, and it is the barrier — taken off the legionary, because a
		...
		"speed": 0.9,
		"health": 1.3,
		"style": CharacterModel.Style.URSAN,
	},
	{
		"name": "SAURIAN",
		...
		"armor": [0, 1, 2],
		"default_armor": 0,
		# SMOKE and the THERMAL HOLO carry their own "kit" key, so the Saurian
		...
		"speed": 1.15,
		"health": 0.95,
		"style": CharacterModel.Style.SAURIAN,
	},

	# =========================================================================
	# DEEP RANGE
	# =========================================================================
	# The COALITION/Hierophany split is done with `primaries`/`secondaries` lists rather
	# than with "kit" keys, because a faction is not one class's private weapon:
	# every COALITION class may reach every COALITION gun, and none of them may touch
	# plasma. That is what an allow-list is for.
	{
		"name": "PALADIN",
		"blurb": "MJOLNIR armour: every COALITION weapon, a shield bubble, and the health to walk in",
		"universe": Universe.DEEP_RANGE,
		"gadgets": [
		...
		"armor": [1, 2, 3],
		"default_armor": 2,
		"primaries": [Weapon.Class.AR7, Weapon.Class.BR3, Weapon.Class.S9_SMG,
			Weapon.Class.CQ12, Weapon.Class.DM3, Weapon.Class.LR99,
			Weapon.Class.HM40, Weapon.Class.RL8, Weapon.Class.LANCE_LASER],
		"secondaries": [Weapon.Class.S6],
		"starter": Weapon.Class.AR7,
		"health": 1.20,
		"style": CharacterModel.Style.PALADIN,
	},
	{
		"name": "DROPTROOPER",
		"blurb": "Orbital drop trooper. Quiet, quick, camouflaged — and thin where the Paladin is not",
		"universe": Universe.DEEP_RANGE,
		"gadgets": [
		...
		"armor": [0, 1, 2],
		"default_armor": 0,
		"primaries": [Weapon.Class.S9_SMG, Weapon.Class.BR3, Weapon.Class.AR7,
			Weapon.Class.DM3, Weapon.Class.LR99],
		"secondaries": [Weapon.Class.S6],
		"starter": Weapon.Class.S9_SMG,
		"speed": 1.12,
		"health": 0.95,
		"style": CharacterModel.Style.DROPTROOPER,
	},
	{
		"name": "ZHAAL",
		"blurb": "Hierophany Elite. Plasma, camouflage, and the sword you close the distance with",
		"universe": Universe.DEEP_RANGE,
		"gadgets": [
		...
		"armor": [0, 1, 2],
		"default_armor": 1,
		"primaries": [Weapon.Class.PLASMA_RIFLE, Weapon.Class.HIER_CARBINE,
		...
		# to cross open ground under fire — the same argument Kinesis adept won.
		"speed": 1.08,
		"health": 1.15,
		"dash": true,
		"style": CharacterModel.Style.ZHAAL,
	},
	{
		"name": "KOPA",
		...
		# only Hierophany body that can carry a fuel rod and still outrun a Paladin.
		"speed": 1.18,
		"health": 0.70,
		"style": CharacterModel.Style.KOPA,
	},
	{
		"name": "URSID",
		...
		"armor": [2, 3],
		"default_armor": 2,
		"primaries": [Weapon.Class.CLEAVER_GUN, Weapon.Class.GRAV_HAMMER,
			Weapon.Class.PLASMA_RIFLE, Weapon.Class.FUEL_ROD],
		"secondaries": [Weapon.Class.MAULER],
		"starter": Weapon.Class.CLEAVER_GUN,
		"speed": 0.92,
		"health": 1.35,
		"style": CharacterModel.Style.URSID,
	},
	...
	# what they do with it: the Sentinel holds a line, the Chorister closes.
	{
		"name": "SENTINEL",
		"blurb": "Codex Order: bolters, plasma, and the discipline to hold a firing line",
		"universe": Universe.IRONHYMN,
		"gadgets": [
			Gadget.NONE, Gadget.AUGUR_SCAN, Gadget.ORBITAL_BOMBARDMENT,
		...
		"armor": [2, 3],
		"default_armor": 2,
		"primaries": [Weapon.Class.SHELLGUN, Weapon.Class.HEAVY_BOLTER,
			Weapon.Class.STALKER_BOLT, Weapon.Class.PLASMA_GUN, Weapon.Class.FUSION_GUN,
			Weapon.Class.FLAMER, Weapon.Class.GRENADE_LAUNCHER,
			Weapon.Class.CHAINSWORD, Weapon.Class.POWER_SWORD],
		"secondaries": [Weapon.Class.BOLT_PISTOL, Weapon.Class.ORDER_PLASMA_PISTOL],
		"starter": Weapon.Class.SHELLGUN,
		"speed": 0.95,
		"health": 1.45,
	...
	{
		"name": "CHORISTER",
		"blurb": "Jump pack and chain blade. It is built to arrive, not to shoot at you from over there",
		"universe": Universe.IRONHYMN,
		"gadgets": [
		...
		"primaries": [Weapon.Class.CHAINSWORD, Weapon.Class.POWER_SWORD,
			Weapon.Class.THUNDER_HAMMER, Weapon.Class.SHELLGUN, Weapon.Class.FLAMER,
			Weapon.Class.FUSION_GUN],
		"secondaries": [Weapon.Class.BOLT_PISTOL, Weapon.Class.ORDER_PLASMA_PISTOL],
		...
		"health": 1.40,
		"dash": true,
		"style": CharacterModel.Style.CHORISTER,
	},
	{
		"name": "UNSLEEPING",
		...
		"armor": [1, 2, 3],
		"default_armor": 2,
		"primaries": [Weapon.Class.GAUSS_RIFLE, Weapon.Class.GAUSS_BLASTER,
		...
		"starter": Weapon.Class.GAUSS_RIFLE,
		# No grenades at all: a Unsleeping reaches out with tesla or it walks at you.
		"speed": 0.92,
		"health": 1.30,
		"style": CharacterModel.Style.UNSLEEPING,
	},
	{
		"name": "SCRAPKIN",
		...
		"universe": Universe.IRONHYMN,
		"gadgets": [
			Gadget.NONE, Gadget.RUNT_GUNNER, Gadget.SCRAP_JETS, Gadget.SCRAP_BOMB,
		...
		"armor": [1, 2, 3],
		"default_armor": 2,
		"primaries": [Weapon.Class.SLUGTHROWER, Weapon.Class.BIG_SHOOTA,
			Weapon.Class.TORCH, Weapon.Class.JUNK_BLASTER, Weapon.Class.SCRAP_ROCKET,
			Weapon.Class.CLEAVER, Weapon.Class.CRUSHER_CLAW],
		"secondaries": [Weapon.Class.SLUG_PISTOL],
		"starter": Weapon.Class.SLUGTHROWER,
		"speed": 1.05,
		"health": 1.25,
		"style": CharacterModel.Style.SCRAPKIN,
	},
]
...
var kit := Kit.LEGION   # index into KITS; picks what the rest of this may be
...
## Magna Guard is a FORCE kit but must look like a droid, not a Warden.
var style := -1
...
## body, and a Aegis Drone moved exactly like a Scout Trooper. Worse, the model's
## `bulk` varied from 0.62 to 1.62 while the CAPSULE never did, so an Kobb
## floated inside an invisible trooper-sized cylinder and a Ursan was 45%
...
## A preset with no `kit` is a LEGIONARY, which is what all the originals
...
const BOT_BUILDS: Array[Dictionary] = [
	...
	# Was the T-21 HMG, which is a Ursan weapon now. The Z-6 is what a legionary
	...
	# SCOUT and SKIRMISHER carry the wrist cable, which is the Hunter's, so
	...
	{"name": "SCOUT", "kit": Kit.HUNTER, "weapon": 5, "secondary": 2,
		"sight": Sight.HOLO,
	...
	# SHOCK used to carry the front shield. That is the Ursan's now, so it
	...
	{"name": "SKIRMISHER", "kit": Kit.HUNTER, "weapon": 1, "secondary": 2,
		"sight": Sight.HOLO,
	...
	{"name": "BOUNTY HUNTER", "kit": Kit.HUNTER, "weapon": 4, "secondary": 1,
	...
	{"name": "DEATH WATCH", "kit": Kit.HUNTER, "weapon": 1, "secondary": 0,
		"sight": Sight.HOLO, "armor": 1, "gadget": Gadget.WRIST_ROCKET,
		"gadget2": Gadget.JETPACK},                                    # 40+20+55+45
	{"name": "DUELLIST", "kit": Kit.ADEPT, "weapon": 12, "secondary": 0,
		"sight": Sight.NONE, "armor": 0, "gadget": Gadget.KINETIC_PUSH,
		"gadget2": Gadget.GRENADE_FRAG},                               # 55+20+45+30
	{"name": "ACOLYTE", "kit": Kit.ADEPT, "weapon": 12, "secondary": 0,
		"sight": Sight.NONE, "armor": 0, "gadget": Gadget.ARC_STORM},  # 55+20+50
	# A Kinesis adept who shoots: a rifle instead of the saber, keeping the pull to
	# drag someone into the open. Fields the new "guns AND powers" build.
	{"name": "WARDEN", "kit": Kit.ADEPT, "weapon": 2, "secondary": 0,
		"sight": Sight.HOLO, "armor": 0, "gadget": Gadget.KINETIC_PULL,
		"gadget2": Gadget.GRENADE_FRAG},                               # 45+20+20+40+30
	# The Saurian fields both of its ideas: a cloaking sniper and a
	# smoke-and-dash rusher. Both legal for the kit; the kit_rules test checks it.
	{"name": "STALKER", "kit": Kit.SAURIAN, "weapon": 10, "secondary": 0,
		"sight": Sight.THERMAL, "armor": 0, "gadget3": Gadget.CLOAK},    # 85+30+20+45
	{"name": "SLAVER", "kit": Kit.SAURIAN, "weapon": 1, "secondary": 2,
		"sight": Sight.THERMAL, "armor": 1, "gadget": Gadget.DASH,
		"gadget2": Gadget.GRENADE_SMOKE},                              # 40+20+30+20
	# The Ursan fields both of its guns, because they play nothing alike: one
	# holds a lane behind the barrier, the other deletes whatever is behind cover.
	# `secondary` 5 is the quarrel caster, which is the only sidearm either may hold —
	# get that index wrong and the kit_rules test says so.
	{"name": "BULWARK", "kit": Kit.URSAN, "weapon": 9, "secondary": 5,
		"sight": Sight.NONE, "armor": 2, "gadget3": Gadget.SHIELD},     # 80+25+50
	{"name": "DEMOLISHER", "kit": Kit.URSAN, "weapon": 11, "secondary": 5,
	...
	{"name": "PALADIN RIFLEMAN", "kit": Kit.PALADIN, "primary": Weapon.Class.AR7,
		"sidearm": Weapon.Class.S6, "sight": Sight.RED_DOT, "armor": 2,
		"gadget2": Gadget.FRAG_GRENADE_UNSC},                          # 45+25+30
	{"name": "BR MARKSMAN", "kit": Kit.PALADIN, "primary": Weapon.Class.BR3,
		"sidearm": Weapon.Class.S6, "sight": Sight.HOLO, "armor": 2,
		"grip": true},                                                 # 55+20+25+20
	{"name": "HEAVY GUNNER", "kit": Kit.PALADIN, "primary": Weapon.Class.HM40,
		"sidearm": Weapon.Class.S6, "sight": Sight.NONE, "armor": 3,
		"cooling": true},                                              # 80+60+20
	{"name": "PALADIN BREACHER", "kit": Kit.PALADIN, "primary": Weapon.Class.CQ12,
		"sidearm": Weapon.Class.S6, "sight": Sight.NONE, "armor": 2,
		"gadget3": Gadget.BUBBLE_SHIELD},                               # 60+25+50
	{"name": "DROP SCOUT", "kit": Kit.DROPTROOPER, "primary": Weapon.Class.S9_SMG,
		"sidearm": Weapon.Class.S6, "sight": Sight.HOLO, "armor": 0,
		"gadget3": Gadget.ACTIVE_CAMO},                                 # 40+20+20+45
	{"name": "DROP SNIPER", "kit": Kit.DROPTROOPER, "primary": Weapon.Class.LR99,
		"sidearm": Weapon.Class.S6, "sight": Sight.SCOPE, "armor": 0,
		"grip": true},                                                 # 85+25+20+20
	# --- DEEP RANGE: Hierophany -------------------------------------------------------
	{"name": "ZHAAL MINOR", "kit": Kit.ZHAAL, "primary": Weapon.Class.PLASMA_RIFLE,
		"sidearm": Weapon.Class.PLASMA_PISTOL, "sight": Sight.HOLO, "armor": 1,
		"gadget2": Gadget.PLASMA_GRENADE},                             # 45+20+35
	{"name": "ZHAAL RANGER", "kit": Kit.ZHAAL, "primary": Weapon.Class.HIER_CARBINE,
		"sidearm": Weapon.Class.PLASMA_PISTOL, "sight": Sight.SCOPE, "armor": 2,
		"gadget": Gadget.THRUSTER_PACK},                               # 55+25+25+25
	{"name": "ZHAAL ZEALOT", "kit": Kit.ZHAAL, "primary": Weapon.Class.ENERGY_SWORD,
	...
	{"name": "KOPA HEAVY", "kit": Kit.KOPA, "primary": Weapon.Class.FUEL_ROD,
		"sidearm": Weapon.Class.PLASMA_PISTOL, "sight": Sight.NONE, "armor": 0,
		"gadget2": Gadget.PLASMA_GRENADE},                             # 105+20+35
	{"name": "URSID CHIEFTAIN", "kit": Kit.URSID, "primary": Weapon.Class.GRAV_HAMMER,
		"sidearm": Weapon.Class.MAULER, "sight": Sight.NONE, "armor": 2,
		"gadget3": Gadget.BUBBLE_SHIELD},                               # 70+30+25+50
	{"name": "URSID GRENADIER", "kit": Kit.URSID, "primary": Weapon.Class.CLEAVER_GUN,
		"sidearm": Weapon.Class.MAULER, "sight": Sight.NONE, "armor": 3,
		"gadget2": Gadget.PLASMA_GRENADE},                             # 70+30+60+35

	# --- IRONHYMN: Order --------------------------------------------------
	{"name": "TACTICAL SENTINEL", "kit": Kit.SENTINEL, "primary": Weapon.Class.SHELLGUN,
		"sidearm": Weapon.Class.BOLT_PISTOL, "sight": Sight.RED_DOT, "armor": 2,
		"gadget2": Gadget.BREACH_CHARGE},                               # 50+15+25+30
	{"name": "BREAKER", "kit": Kit.SENTINEL, "primary": Weapon.Class.HEAVY_BOLTER,
		"sidearm": Weapon.Class.BOLT_PISTOL, "sight": Sight.NONE, "armor": 3,
		"cooling": true},                                              # 80+60+20
	{"name": "OATHGUARD", "kit": Kit.SENTINEL, "primary": Weapon.Class.PLASMA_GUN,
		"sidearm": Weapon.Class.BOLT_PISTOL, "sight": Sight.HOLO, "armor": 2,
		"gadget": Gadget.AUGUR_SCAN},                                 # 90+20+25+30
	{"name": "RECON SENTINEL", "kit": Kit.SENTINEL, "primary": Weapon.Class.STALKER_BOLT,
		"sidearm": Weapon.Class.BOLT_PISTOL, "sight": Sight.SCOPE_4X, "armor": 2,
		"grip": true},                                                 # 80+35+25+20
	{"name": "ASSAULT CHORISTER", "kit": Kit.CHORISTER, "primary": Weapon.Class.CHAINSWORD,
		"sidearm": Weapon.Class.BOLT_PISTOL, "sight": Sight.NONE, "armor": 2,
		"gadget": Gadget.JUMP_PACK, "gadget2": Gadget.BREACH_CHARGE},   # 45+25+45+30
	{"name": "LOST COMPANY", "kit": Kit.CHORISTER, "primary": Weapon.Class.THUNDER_HAMMER,
		"sidearm": Weapon.Class.BOLT_PISTOL, "sight": Sight.NONE, "armor": 3,
		"gadget3": Gadget.CRIMSON_RAGE},                                  # 75+60+25
	# --- IRONHYMN: Necrons ---------------------------------------------------
	{"name": "UNSLEEPING WARRIOR", "kit": Kit.UNSLEEPING, "primary": Weapon.Class.GAUSS_RIFLE,
		"sidearm": Weapon.Class.GAUSS_PISTOL, "sight": Sight.RED_DOT, "armor": 2,
		"cooling": true},                                              # 45+15+25+20
	{"name": "REVENANT", "kit": Kit.UNSLEEPING, "primary": Weapon.Class.GAUSS_BLASTER,
		"sidearm": Weapon.Class.GAUSS_PISTOL, "sight": Sight.HOLO, "armor": 3,
		"grip": true},                                                 # 60+20+60+20
	{"name": "SILENT MARK", "kit": Kit.UNSLEEPING, "primary": Weapon.Class.SYNAPTIC_DISINTEGRATOR,
		"sidearm": Weapon.Class.GAUSS_PISTOL, "sight": Sight.SCOPE, "armor": 1,
		"gadget3": Gadget.PHASE_SHIFT},                                 # 85+25+45
	{"name": "SEPULCHRE GUARD", "kit": Kit.UNSLEEPING, "primary": Weapon.Class.WARSCYTHE,
	...
	# --- IRONHYMN: Orks ------------------------------------------------------
	{"name": "SCATTER BRAWLER", "kit": Kit.SCRAPKIN, "primary": Weapon.Class.SLUGTHROWER,
		"sidearm": Weapon.Class.SLUG_PISTOL, "sight": Sight.NONE, "armor": 2,
		"gadget3": Gadget.WARCRY, "gadget2": Gadget.SCRAP_BOMB},         # 45+25+25+25
	{"name": "TORCH BRAWLER", "kit": Kit.SCRAPKIN, "primary": Weapon.Class.TORCH,
		"sidearm": Weapon.Class.SLUG_PISTOL, "sight": Sight.NONE, "armor": 2,
		"gadget3": Gadget.KUSTOM_FORCE_FIELD},                          # 55+25+50
	{"name": "HULL CRACKER", "kit": Kit.SCRAPKIN, "primary": Weapon.Class.SCRAP_ROCKET,
		"sidearm": Weapon.Class.SLUG_PISTOL, "sight": Sight.NONE, "armor": 2,
		"gadget2": Gadget.SCRAP_BOMB},                                  # 105+25+25
	{"name": "SCRAP BOSS", "kit": Kit.SCRAPKIN, "primary": Weapon.Class.CRUSHER_CLAW,
		"sidearm": Weapon.Class.SLUG_PISTOL, "sight": Sight.NONE, "armor": 3,
		"gadget3": Gadget.WARCRY, "style": CharacterModel.Style.SCRAPKIN_BOSS},  # 70+60+25
	{"name": "TINKER GUNNER", "kit": Kit.SCRAPKIN, "primary": Weapon.Class.BIG_SHOOTA,
		"sidearm": Weapon.Class.SLUG_PISTOL, "sight": Sight.NONE, "armor": 3,
		"gadget": Gadget.RUNT_GUNNER},                                 # 70+60+65
]
...
static func universe_builds() -> Array[int]:
	...
	for i in BOT_BUILDS.size():
		if kit_universe(int(BOT_BUILDS[i].get("kit", Kit.LEGION))) == active_universe:
			out.append(i)
	return out
...
##
## Order matters: the first four are CONCORD, the last four AUTOMATA;
...
const FACTION_BUILDS: Array[Dictionary] = [
	# CONCORD ---------------------------------------------------------------
	# These eight were authored before the Dominion and Pact rosters, and it
	# showed: several carried ONE ability where every later class carries three,
	# so the Compact Wars sides were quietly the weakest thing to play. They also
	# threw a "FRAG GRENADE" — The Compact Wars has a noun for that, and it is the same
	# gadget at the same price (THERMAL_CHARGE is an alias of GRENADE_FRAG), so
	...
	# those numbers were per-KIT only, so all sixteen Compact Wars classes on the
	# default kit were the SAME BODY: a Aegis Drone moved at exactly a Scout
	# Trooper's pace, and a light automaton took exactly as much killing as a Legion Commando.
	# The armour frame was the only thing separating any of them, and it is a
	# purchase, not a character. The LEGIONARY is the yardstick and states
	...
	# it was on all thirty-two The Compact Wars classes: a light automaton battle droid, a Vespid
	# and an Dominion Royal Guard all drew Han Solo's pistol. Deep Range and Ironhymn
	# each hand every faction its own free sidearm (M6D, plasma pistol, bolt
	# pistol, gauss pistol, slugga) and The Compact Wars simply never did, because it
	# had one faction when index 0 was chosen and now has four. Concord legionaries
	# carry the DC-17 hand blaster, the Separatists the SE-14, the Dominion the
	# SE-14r and the the Pact the DH-17 — and the DL-44 stays where it belongs,
	# with the Pact Pathfinder. Faction presets are authored classes rather
	# than shop builds, so the prices on those rows do not apply to them.
	{"name": "LEGIONARY", "weapon": 2, "sight": Sight.RED_DOT, "cooling": true,
		"sidearm": Weapon.Class.VL17, "armor": 1, "gadget2": Gadget.THERMAL_CHARGE,
		"gadget3": Gadget.OVERSHIELD, "style": CharacterModel.Style.LEGION},
	{"name": "LEGION TECH", "weapon": 6, "grip": true,
		"sidearm": Weapon.Class.VL17,
		"armor": 2, "gadget": Gadget.TURRET, "gadget2": Gadget.GRENADE_SMOKE,
		"gadget3": Gadget.DEPLOY_COVER, "unit_speed": 1.03, "unit_health": 0.95,
		"style": CharacterModel.Style.LEGION_TECH},
	{"name": "LEGION HEAVY", "weapon": 9, "foregrip": true, "sight": Sight.RED_DOT,
		"sidearm": Weapon.Class.VL17, "armor": 3, "gadget": Gadget.MORTAR,
		"gadget2": Gadget.THERMAL_CHARGE, "gadget3": Gadget.SHIELD,
		"unit_speed": 0.88, "unit_health": 1.25, "unit_jump": 0.90,
		"unit_stature": 1.06, "style": CharacterModel.Style.LEGION_HEAVY},
	...
	{"name": "LEGION SPECIALIST", "weapon": 2, "sight": Sight.RED_DOT, "grip": true,
		"sidearm": Weapon.Class.VL17, "secondary_mod": SecondaryMod.DUAL, "armor": 1,
		"gadget": Gadget.SCAN_DART, "gadget2": Gadget.THERMAL_CHARGE,
		"gadget3": Gadget.CLOAK, "unit_speed": 1.10, "unit_health": 0.90,
		"unit_jump": 1.06, "style": CharacterModel.Style.LEGION_VANGUARD},
	# AUTOMATA -------------------------------------------------------------
	# A light automaton IS SUPPOSED TO DIE. It is the only unit in the game built around
	...
	# that wins with a light automaton wins because there were more of them.
	{"name": "AUTOMATON", "weapon": 2, "sight": Sight.HOLO, "foregrip": true,
		"sidearm": Weapon.Class.REVOLVER, "armor": 0, "gadget2": Gadget.THERMAL_CHARGE,
		"gadget3": Gadget.OVERSHIELD, "unit_speed": 0.96, "unit_health": 0.84,
		"unit_stature": 0.94, "style": CharacterModel.Style.AUTOMATON},
	# No grenade, on purpose: a heavy automaton has no hands free for one. Everything it
	# fights with is bolted to its arms — and it walks like it. Slowest thing
	# on the Automata roster that is not a Aegis Drone, and the hardest to drop.
	{"name": "HEAVY AUTOMATON", "primary_override": Weapon.Class.WRIST_CANNON,
		...
		"unit_stature": 1.15, "style": CharacterModel.Style.AUTOMATON_HEAVY},
	...
	{"name": "GLAIVE DRONE", "kit": Kit.ADEPT, "primary_override": Weapon.Class.STAFF,
		...
		"unit_stature": 1.04, "style": CharacterModel.Style.GLAIVE_DRONE},
	{"name": "TACTICAL DRONE", "weapon": NO_PRIMARY,
		"sidearm": Weapon.Class.REVOLVER,
		"secondary_mod": SecondaryMod.SCOPE, "armor": 1,
		"gadget": Gadget.MORTAR, "gadget2": Gadget.THERMAL_CHARGE,
	...
	# was laid out around a 1.8 m body — a unit tall enough to be a real Paladin
	...
	{"name": "PALADIN-II", "kit": Kit.PALADIN, "primary": Weapon.Class.AR7,
		"sidearm": Weapon.Class.S6, "sight": Sight.RED_DOT, "armor": 2,
		"gadget3": Gadget.BUBBLE_SHIELD, "gadget2": Gadget.FRAG_GRENADE_UNSC,
		"unit_speed": 1.05, "unit_health": 1.40, "unit_stature": 1.12,
		"style": CharacterModel.Style.PALADIN},
	{"name": "PALADIN HEAVY", "kit": Kit.PALADIN, "primary": Weapon.Class.HM40,
		"sidearm": Weapon.Class.S6, "cooling": true, "armor": 3,
		...
		# LINE class. A Paladin Heavy should be the tough one on its own side, not
		# tougher than the Hierophany's heavies as well.
		"unit_speed": 0.94, "unit_health": 1.28, "unit_jump": 0.90,
		"unit_stature": 1.14, "style": CharacterModel.Style.PALADIN},
	{"name": "DROPTROOPER", "kit": Kit.DROPTROOPER, "primary": Weapon.Class.S9_SMG,
		"sidearm": Weapon.Class.S6, "sight": Sight.HOLO, "armor": 0,
		"gadget3": Gadget.ACTIVE_CAMO, "gadget2": Gadget.FRAG_GRENADE_UNSC,
		"unit_speed": 1.12, "unit_health": 0.95, "unit_jump": 1.08,
		"style": CharacterModel.Style.DROPTROOPER},
	{"name": "COALITION MARKSMAN", "kit": Kit.DROPTROOPER, "primary": Weapon.Class.DM3,
		"sidearm": Weapon.Class.S6, "grip": true, "armor": 1,
		"gadget": Gadget.TRACKER_DART, "gadget2": Gadget.FRAG_GRENADE_UNSC,
		"unit_speed": 1.00, "unit_health": 0.90, "unit_stature": 0.98,
		"style": CharacterModel.Style.COALITION_MARINE},
	# HALO — Hierophany (12-15) -------------------------------------------------
	{"name": "ZHAAL MINOR", "kit": Kit.ZHAAL, "primary": Weapon.Class.PLASMA_RIFLE,
		"sidearm": Weapon.Class.PLASMA_PISTOL, "sight": Sight.HOLO, "armor": 1,
	...
	# with a Paladin, which took the joke out of the only comedy unit in Deep Range
	...
	{"name": "URSID CHIEFTAIN", "kit": Kit.URSID, "primary": Weapon.Class.GRAV_HAMMER,
		...
		"unit_stature": 1.16, "style": CharacterModel.Style.URSID},

	# IRONHYMN — Ultramarines (16-19) ----------------------------------------
	# AN ORDER IS NOT A MAN IN ARMOUR. Every one of these is taller, heavier
	...
	# Sentinel's speed is supposed to come out of a rocket, not out of legs.
	{"name": "TACTICAL SENTINEL", "kit": Kit.SENTINEL, "primary": Weapon.Class.SHELLGUN,
		"sidearm": Weapon.Class.BOLT_PISTOL, "sight": Sight.RED_DOT, "armor": 2,
		"gadget3": Gadget.IRON_HALO, "gadget2": Gadget.BREACH_CHARGE,
		"unit_speed": 0.92, "unit_health": 1.35, "unit_jump": 0.95,
		"unit_stature": 1.14, "style": CharacterModel.Style.SENTINEL},
	{"name": "BREAKER", "kit": Kit.SENTINEL, "primary": Weapon.Class.HEAVY_BOLTER,
		"sidearm": Weapon.Class.BOLT_PISTOL, "cooling": true, "armor": 3,
		"gadget": Gadget.ORBITAL_BOMBARDMENT, "gadget2": Gadget.BREACH_CHARGE,
		"unit_speed": 0.82, "unit_health": 1.60, "unit_jump": 0.85,
		"unit_stature": 1.16, "style": CharacterModel.Style.SENTINEL},
	{"name": "OATHGUARD", "kit": Kit.SENTINEL, "primary": Weapon.Class.PLASMA_GUN,
		"sidearm": Weapon.Class.ORDER_PLASMA_PISTOL, "sight": Sight.HOLO, "armor": 2,
		"gadget": Gadget.AUGUR_SCAN, "gadget2": Gadget.FUSION_CHARGE,
	...
	{"name": "VANGUARD ADEPT", "kit": Kit.SENTINEL, "primary": Weapon.Class.POWER_SWORD,
		...
		"style": CharacterModel.Style.SENTINEL},
	# IRONHYMN — Blood Angels (20-23) ----------------------------------------
	{"name": "ASSAULT CHORISTER", "kit": Kit.CHORISTER, "primary": Weapon.Class.CHAINSWORD,
		"sidearm": Weapon.Class.BOLT_PISTOL, "armor": 2,
		"gadget": Gadget.JUMP_PACK, "gadget2": Gadget.BREACH_CHARGE,
		"unit_speed": 1.08, "unit_health": 1.20, "unit_jump": 1.15,
		"unit_stature": 1.12, "style": CharacterModel.Style.CHORISTER},
	{"name": "CHOIR GUARD", "kit": Kit.CHORISTER, "primary": Weapon.Class.POWER_SWORD,
		"sidearm": Weapon.Class.ORDER_PLASMA_PISTOL, "armor": 3,
		"gadget": Gadget.JUMP_PACK, "gadget3": Gadget.CRIMSON_RAGE,
		"unit_speed": 1.10, "unit_health": 1.35, "unit_jump": 1.20,
		"unit_stature": 1.14, "style": CharacterModel.Style.CHORISTER},
	{"name": "LOST COMPANY", "kit": Kit.CHORISTER, "primary": Weapon.Class.THUNDER_HAMMER,
		"sidearm": Weapon.Class.BOLT_PISTOL, "armor": 3,
		"gadget3": Gadget.CRIMSON_RAGE, "gadget2": Gadget.FUSION_CHARGE,
		"unit_speed": 1.06, "unit_health": 1.45, "unit_stature": 1.14,
		"style": CharacterModel.Style.CHORISTER},
	{"name": "CHOIR TACTICAL", "kit": Kit.CHORISTER, "primary": Weapon.Class.SHELLGUN,
		"sidearm": Weapon.Class.BOLT_PISTOL, "sight": Sight.HOLO, "armor": 2,
		"gadget3": Gadget.IRON_HALO, "gadget2": Gadget.BREACH_CHARGE,
		"unit_speed": 0.94, "unit_health": 1.30, "unit_stature": 1.14,
		"style": CharacterModel.Style.CHORISTER},
	# IRONHYMN — Necrons (24-27) ---------------------------------------------
	...
	# Nothing new was needed to fix it — a Unsleeping already owns a teleport, a
	...
	{"name": "UNSLEEPING WARRIOR", "kit": Kit.UNSLEEPING, "primary": Weapon.Class.GAUSS_RIFLE,
		...
		"unit_stature": 1.08, "style": CharacterModel.Style.UNSLEEPING},
	{"name": "REVENANT", "kit": Kit.UNSLEEPING, "primary": Weapon.Class.GAUSS_BLASTER,
		"sidearm": Weapon.Class.GAUSS_PISTOL, "sight": Sight.HOLO, "armor": 3,
		"gadget": Gadget.SENTRY_CONSTRUCT, "gadget3": Gadget.OVERSHIELD,
		"unit_speed": 0.82, "unit_health": 1.55, "unit_jump": 0.80,
		"unit_stature": 1.12, "style": CharacterModel.Style.UNSLEEPING},
	{"name": "SILENT MARK", "kit": Kit.UNSLEEPING, "primary": Weapon.Class.SYNAPTIC_DISINTEGRATOR,
	...
	{"name": "SEPULCHRE GUARD", "kit": Kit.UNSLEEPING, "primary": Weapon.Class.WARSCYTHE,
		...
		"unit_stature": 1.14, "style": CharacterModel.Style.UNSLEEPING_LORD},
	# IRONHYMN — Orks (28-31) ------------------------------------------------
	...
	{"name": "SCATTER BRAWLER", "kit": Kit.SCRAPKIN, "primary": Weapon.Class.SLUGTHROWER,
		"sidearm": Weapon.Class.SLUG_PISTOL, "armor": 2,
		"gadget3": Gadget.WARCRY, "gadget2": Gadget.SCRAP_BOMB,
	...
	{"name": "HULL CRACKER", "kit": Kit.SCRAPKIN, "primary": Weapon.Class.SCRAP_ROCKET,
		"sidearm": Weapon.Class.SLUG_PISTOL, "armor": 2,
		"gadget": Gadget.SCRAP_JETS, "gadget2": Gadget.SCRAP_BOMB,
		"unit_speed": 0.96, "unit_health": 1.20, "unit_stature": 1.08,
		"style": CharacterModel.Style.SCRAPKIN},
	{"name": "SCRAP BOSS", "kit": Kit.SCRAPKIN, "primary": Weapon.Class.CRUSHER_CLAW,
		"sidearm": Weapon.Class.SLUG_PISTOL, "armor": 3,
		"gadget": Gadget.RUNT_GUNNER, "gadget3": Gadget.WARCRY,
		"unit_speed": 0.92, "unit_health": 1.60, "unit_jump": 0.90,
		"unit_stature": 1.16, "style": CharacterModel.Style.SCRAPKIN_BOSS},
	# =========================================================================
	# THE SECOND HALF OF EVERY ROSTER (32+), plus two new The Compact Wars sides.
	#
	# APPENDED, never interleaved. FACTION_ROSTERS names classes by INDEX, so
	# inserting a Concord class next to the other Concord ones would silently
	# re-deal every roster below it — the same trap the WEAPONS table has and the
	# same answer: add at the end and list the index.
	# =========================================================================

	# =========================================================================
	# THE THE COMPACT WARS ROSTERS, REBUILT. The Genre's own vocabulary: every side is
	...
	# CONCORD, second four (32-35) ------------------------------------------
	{"name": "LEGION OFFICER", "primary": Weapon.Class.VL15S,
		"sight": Sight.RED_DOT, "sidearm": Weapon.Class.VL17, "armor": 1, "squad": 2,
		"gadget": Gadget.TURRET, "gadget2": Gadget.THERMAL_CHARGE,
		"gadget3": Gadget.RALLY, "unit_speed": 1.02,
		"style": CharacterModel.Style.LEGION_TECH},
	{"name": "LEGION SHARPSHOOTER", "primary": Weapon.Class.VL15X,
		"sidearm": Weapon.Class.VL17, "armor": 0, "gadget": Gadget.PULSE_SCAN,
		"gadget2": Gadget.GRENADE_SMOKE, "gadget3": Gadget.CLOAK,
		"unit_speed": 1.08, "unit_health": 0.85, "unit_stature": 0.98,
		"style": CharacterModel.Style.LEGION_VANGUARD},
	...
	# is the fastest thing the Concord fields and the Commando the toughest.
	{"name": "VANGUARD TROOPER", "weapon": NO_PRIMARY,
		"sidearm": Weapon.Class.VL17,
		...
		"unit_jump": 1.10, "style": CharacterModel.Style.LEGION_VANGUARD},
	{"name": "LEGION COMMANDO", "primary": Weapon.Class.VL17M,
		"sight": Sight.RED_DOT, "sidearm": Weapon.Class.VL17, "armor": 3,
		"gadget": Gadget.SCAN_DART, "gadget2": Gadget.THERMAL_CHARGE,
		"gadget3": Gadget.OVERSHIELD, "unit_speed": 0.95, "unit_health": 1.35,
		"unit_stature": 1.05, "style": CharacterModel.Style.LEGION_COMMANDO},

	# AUTOMATA, second four (36-39) ----------------------------------------
	{"name": "SNIPER DRONE", "primary": Weapon.Class.X5S,
		"sidearm": Weapon.Class.REVOLVER, "armor": 0, "gadget": Gadget.PULSE_SCAN,
		"gadget2": Gadget.GRENADE_SMOKE, "gadget3": Gadget.CLOAK,
		"unit_speed": 0.96, "unit_health": 0.84, "unit_stature": 0.94,
		"style": CharacterModel.Style.AUTOMATON},
	# AEGIS DRONE: the twin repeaters and the deflector bubble, which is the unit —
	...
	{"name": "AEGIS DRONE", "primary": Weapon.Class.AEGIS_TWIN,
		...
		"unit_stature": 0.88, "style": CharacterModel.Style.AEGIS_DRONE},
	{"name": "INFILTRATOR DRONE", "kit": Kit.ADEPT, "weapon": NO_PRIMARY,
		...
		"unit_jump": 1.15, "style": CharacterModel.Style.INFILTRATOR_DRONE},
	# Light enough to fly and light enough to swat: the highest jump on the
	# roster over the second-thinnest body.
	{"name": "VESPID WARRIOR", "primary": Weapon.Class.SONIC_BLASTER,
		"sidearm": Weapon.Class.REVOLVER, "armor": 0, "gadget": Gadget.WINGS,
		"gadget2": Gadget.THERMAL_CHARGE, "gadget3": Gadget.FURY,
		"unit_speed": 1.10, "unit_health": 0.82, "unit_jump": 1.35,
		"unit_stature": 0.94, "style": CharacterModel.Style.VESPID},

	# DOMINION (40-47) ----------------------------------------------------------
	{"name": "DOMINION TROOPER", "primary": Weapon.Class.DK11,
		"sight": Sight.RED_DOT, "sidearm": Weapon.Class.SR14R, "armor": 1,
		"gadget2": Gadget.THERMAL_CHARGE, "gadget3": Gadget.OVERSHIELD,
		"style": CharacterModel.Style.DOMINION_TROOPER},
	{"name": "DOMINION HEAVY", "primary": Weapon.Class.DK19,
		"foregrip": true, "sidearm": Weapon.Class.SR14R, "armor": 3,
		"gadget2": Gadget.THERMAL_CHARGE, "gadget3": Gadget.DEPLOY_COVER,
		"unit_speed": 0.88, "unit_health": 1.28, "unit_jump": 0.90,
		"unit_stature": 1.06, "style": CharacterModel.Style.DOMINION_HEAVY},
	# The officers each carry their OWN side's rifle. All three used to be
	# listed with the DC-15S — a Concord legionary carbine in the hands of an
	# Dominion and a Pact officer, twenty years and one war on the wrong side.
	{"name": "DOMINION OFFICER", "primary": Weapon.Class.DK11,
		"sidearm": Weapon.Class.SR14R, "armor": 1, "squad": 2, "gadget": Gadget.TURRET,
		"gadget2": Gadget.GRENADE_SMOKE, "gadget3": Gadget.RALLY,
		"unit_speed": 1.02, "unit_health": 0.92,
		"style": CharacterModel.Style.DOMINION_OFFICER},
	# The fastest line class either Dominion or Pact fields, and the softest.
	# A scout is a flanker or it is nothing, and it has to be genuinely
	# punishing to get caught in the open as one.
	{"name": "DOMINION SCOUT", "primary": Weapon.Class.DK20A,
		"sidearm": Weapon.Class.SR14R, "armor": 0, "gadget": Gadget.PULSE_SCAN,
		"gadget2": Gadget.GRENADE_SMOKE, "gadget3": Gadget.CLOAK,
		"unit_speed": 1.18, "unit_health": 0.82, "unit_jump": 1.18,
		"unit_stature": 0.97, "style": CharacterModel.Style.DOMINION_SCOUT},
	# The reinforcements: the two units the Dominion is actually feared for, plus
	...
	{"name": "REAPER TROOPER", "kit": Kit.SAURIAN,
		"primary": Weapon.Class.DK11D, "sight": Sight.THERMAL,
		"sidearm": Weapon.Class.SR14R, "armor": 2, "gadget": Gadget.SCAN_DART,
		"gadget2": Gadget.GRENADE_SMOKE, "gadget3": Gadget.CLOAK,
		"unit_speed": 1.06, "unit_health": 1.20, "unit_stature": 1.03,
		"style": CharacterModel.Style.REAPER_TROOPER},
	...
	# arriving at all is.
	{"name": "INCINERATOR TROOPER", "primary": Weapon.Class.FLAMETHROWER,
		"sidearm": Weapon.Class.SR14R, "armor": 2, "gadget2": Gadget.THERMAL_CHARGE,
		"gadget3": Gadget.OVERSHIELD, "unit_speed": 0.94, "unit_health": 1.30,
		"unit_stature": 1.04, "style": CharacterModel.Style.INCINERATOR_TROOPER},
	{"name": "GARRISON TROOPER", "primary": Weapon.Class.DK11,
		"grip": true, "sidearm": Weapon.Class.SR14R, "armor": 2, "gadget": Gadget.MORTAR,
		"gadget2": Gadget.THERMAL_CHARGE, "gadget3": Gadget.DEPLOY_COVER,
		"unit_speed": 0.96, "unit_health": 1.15, "unit_stature": 1.02,
		"style": CharacterModel.Style.GARRISON_TROOPER},
	...
	# hardest body in The Compact Wars to put down.
	{"name": "DOMINION GUARD", "kit": Kit.ADEPT, "weapon": NO_PRIMARY,
		"primary_override": Weapon.Class.STAFF,
		"sidearm": Weapon.Class.SR14R, "armor": 3,
		"gadget": Gadget.DASH, "gadget3": Gadget.FURY,
		"unit_speed": 0.84, "unit_health": 1.70, "unit_jump": 0.85,
		"unit_stature": 1.08, "style": CharacterModel.Style.DOMINION_GUARD},

	# PACT ALLIANCE (48-55) --------------------------------------------------
	{"name": "PACT TROOPER", "primary": Weapon.Class.A28C,
		"sight": Sight.RED_DOT, "sidearm": Weapon.Class.DH7, "armor": 1,
		"gadget2": Gadget.THERMAL_CHARGE, "gadget3": Gadget.OVERSHIELD,
		"style": CharacterModel.Style.PACT_TROOPER},
	{"name": "PACT VANGUARD", "primary": Weapon.Class.CR9,
		"sidearm": Weapon.Class.DH7, "armor": 2, "gadget": Gadget.DASH,
		"gadget2": Gadget.THERMAL_CHARGE, "gadget3": Gadget.FURY,
		"unit_speed": 1.05, "unit_health": 1.15, "unit_stature": 1.05,
		"style": CharacterModel.Style.PACT_VANGUARD},
	{"name": "PACT OFFICER", "primary": Weapon.Class.A28C,
		"sidearm": Weapon.Class.DH7, "armor": 1, "squad": 2, "gadget": Gadget.TURRET,
		"gadget2": Gadget.GRENADE_SMOKE, "gadget3": Gadget.RALLY,
		"unit_speed": 1.02, "unit_health": 0.92,
		"style": CharacterModel.Style.PACT_OFFICER},
	{"name": "PACT MARKSMAN", "primary": Weapon.Class.DH44,
		"sidearm": Weapon.Class.DH7, "armor": 0, "gadget": Gadget.PULSE_SCAN,
		"gadget2": Gadget.GRENADE_SMOKE, "gadget3": Gadget.CLOAK,
		"unit_speed": 1.08, "unit_health": 0.85, "unit_stature": 0.98,
		"style": CharacterModel.Style.PACT_PILOT},
	# Reinforcements: the Ursan and the Kobb are the two the fanbase asks for
	...
	# and hardest body in The Compact Wars against the smallest and softest.
	# THE QUARREL CASTER, named rather than left to index 0. `secondary: 0` is the
	...
	# Ursan kit actually sells it; what a player sees is the gun in its hands.
	{"name": "URSAN WARRIOR", "kit": Kit.URSAN, "weapon": NO_PRIMARY,
		"sidearm": Weapon.Class.QUARREL_CASTER,
		"armor": 3, "gadget2": Gadget.THERMAL_CHARGE, "gadget3": Gadget.FURY,
		"unit_speed": 0.86, "unit_health": 1.55, "unit_jump": 0.85,
		"unit_stature": 1.16, "style": CharacterModel.Style.URSAN},
	# KNEE-HIGH AND LETHAL. The stature is not decoration: at 0.62 the capsule
	# is about 1.1 m, so an Kobb is genuinely hard to hit, genuinely fast, and
	...
	# until now the joke was purely visual and it fought like a dominion trooper.
	{"name": "KOBB HUNTER", "kit": Kit.ADEPT, "weapon": NO_PRIMARY,
		# The holdout: an Kobb's blaster is a thing it took off somebody.
		"primary_override": Weapon.Class.KOBB_SPEAR,
		...
		"style": CharacterModel.Style.KOBB},
	{"name": "PACT PATHFINDER", "kit": Kit.SAURIAN,
		# The one class that keeps the DL-44 — a Pathfinder is the Alliance's
		# scoundrel, and it is his pistol.
		"primary": Weapon.Class.A28C, "sight": Sight.THERMAL,
		"sidearm": Weapon.Class.PISTOL, "armor": 1, "gadget": Gadget.SCAN_DART,
		"gadget2": Gadget.GRENADE_SMOKE, "gadget3": Gadget.CLOAK,
		"unit_speed": 1.10, "unit_health": 0.90, "unit_jump": 1.08,
		"style": CharacterModel.Style.PACT_COMMANDO},
	{"name": "PACT WARDEN", "kit": Kit.ADEPT, "weapon": NO_PRIMARY,
		"primary_override": Weapon.Class.SABER,
		"sidearm": Weapon.Class.BR9, "armor": 1,
		"gadget": Gadget.KINETIC_PUSH, "gadget2": Gadget.KINETIC_LEAP,
		"gadget3": Gadget.FURY, "unit_speed": 1.22, "unit_health": 1.30,
		"unit_jump": 1.20, "style": CharacterModel.Style.WARDEN},

	# =========================================================================
	# DEEP RANGE, REBUILT. The sandbox IS the roster here — a Deep Range class is defined by
	# which of the iconic guns it walks in with, so every one of these names a
	# different weapon and no two share a primary.
	# =========================================================================

	# HALO — COALITION, second four (56-59) ----------------------------------------
	{"name": "PALADIN CQC", "kit": Kit.PALADIN, "primary": Weapon.Class.CQ12,
		"sidearm": Weapon.Class.S6, "armor": 2, "gadget": Gadget.THRUSTER_PACK,
		"gadget2": Gadget.FRAG_GRENADE_UNSC, "gadget3": Gadget.OVERSHIELD,
		"unit_speed": 1.10, "unit_health": 1.30, "unit_stature": 1.12,
		"style": CharacterModel.Style.PALADIN},
	{"name": "DROP SNIPER", "kit": Kit.DROPTROOPER, "primary": Weapon.Class.LR99,
		"sidearm": Weapon.Class.S6, "armor": 0, "gadget": Gadget.HELMET_SCAN,
		"gadget2": Gadget.FRAG_GRENADE_UNSC, "gadget3": Gadget.ACTIVE_CAMO,
		"unit_speed": 1.06, "unit_health": 0.90, "unit_stature": 0.98,
		"style": CharacterModel.Style.DROPTROOPER},
	{"name": "COALITION ROCKETEER", "kit": Kit.DROPTROOPER, "primary": Weapon.Class.RL8,
		"sidearm": Weapon.Class.S6, "armor": 1, "gadget2": Gadget.FRAG_GRENADE_UNSC,
		"gadget3": Gadget.DEPLOY_COVER, "unit_speed": 0.94, "unit_health": 1.05,
		"style": CharacterModel.Style.COALITION_MARINE},
	{"name": "COALITION GRENADIER", "kit": Kit.DROPTROOPER, "primary": Weapon.Class.GL19,
		"sidearm": Weapon.Class.S6, "armor": 1, "gadget": Gadget.SENTRY_TURRET,
		"gadget2": Gadget.FRAG_GRENADE_UNSC, "gadget3": Gadget.BIOFOAM,
		"unit_speed": 0.98, "unit_health": 0.95,
		"style": CharacterModel.Style.COALITION_MARINE},

	# HALO — Hierophany, second four (60-63) ------------------------------------
	{"name": "ZHAAL ULTRA", "kit": Kit.ZHAAL, "primary": Weapon.Class.HIER_CARBINE,
		...
		"style": CharacterModel.Style.ZHAAL_ULTRA},
	{"name": "ZHAAL RANGER", "kit": Kit.ZHAAL, "primary": Weapon.Class.BEAM_RIFLE,
		...
		"style": CharacterModel.Style.ZHAAL},
	# THE SKIRI: a beam rifle behind a gauntlet you cannot shoot through. The
	# deflector is the unit, exactly as it is for the Aegis Drone — and like the
	# Grunt it is a small, light body, which is most of why a Kig-Yar behind
	# that shield is so annoying to dig out.
	{"name": "SKIRI SNIPER", "kit": Kit.KOPA, "primary": Weapon.Class.BEAM_RIFLE,
		"sidearm": Weapon.Class.PLASMA_PISTOL, "armor": 1,
		"gadget2": Gadget.PLASMA_GRENADE, "gadget3": Gadget.DEFLECTOR,
		"unit_speed": 1.12, "unit_health": 0.78, "unit_stature": 0.88,
		"style": CharacterModel.Style.SKIRI},
	# The Chieftain already holds the hammer in the first four, so this is the
	# other half of the Ursid fantasy: a stalker with the spiker and camo.
	{"name": "URSID STALKER", "kit": Kit.URSID, "primary": Weapon.Class.SPIKER,
	...
	# IRONHYMN — Ultramarines, second four (64-67) ---------------------------------
	{"name": "ASSAULT SENTINEL", "kit": Kit.SENTINEL, "primary": Weapon.Class.CHAINSWORD,
		"sidearm": Weapon.Class.BOLT_PISTOL, "armor": 2,
		"gadget": Gadget.JUMP_PACK, "gadget2": Gadget.BREACH_CHARGE,
		"gadget3": Gadget.IRON_HALO, "unit_speed": 1.02, "unit_health": 1.30,
		"unit_stature": 1.12, "style": CharacterModel.Style.SENTINEL},
	{"name": "PLASMA SENTINEL", "kit": Kit.SENTINEL, "primary": Weapon.Class.PLASMA_GUN,
		"sidearm": Weapon.Class.BOLT_PISTOL, "cooling": true, "armor": 3,
		"gadget2": Gadget.BREACH_CHARGE, "gadget3": Gadget.OVERSHIELD,
		"unit_speed": 0.86, "unit_health": 1.50, "unit_stature": 1.14,
		"style": CharacterModel.Style.SENTINEL},
	{"name": "MEDICAE", "kit": Kit.SENTINEL, "primary": Weapon.Class.SHELLGUN,
		"sidearm": Weapon.Class.BOLT_PISTOL, "armor": 2, "squad": 2,
		"gadget": Gadget.AUGUR_SCAN, "gadget2": Gadget.BREACH_CHARGE,
		"gadget3": Gadget.IRON_HALO, "unit_speed": 0.94, "unit_health": 1.35,
		"unit_stature": 1.14, "style": CharacterModel.Style.SENTINEL},
	{"name": "FUSION SENTINEL", "kit": Kit.SENTINEL, "primary": Weapon.Class.FUSION_GUN,
		"sidearm": Weapon.Class.BOLT_PISTOL, "armor": 3,
		"gadget2": Gadget.FUSION_CHARGE, "gadget3": Gadget.OVERSHIELD,
		"unit_speed": 0.84, "unit_health": 1.55, "unit_stature": 1.16,
		"style": CharacterModel.Style.SENTINEL},

	# IRONHYMN — Blood Angels, second four (68-71) ---------------------------------
	{"name": "CHOIR PRIEST", "kit": Kit.CHORISTER, "primary": Weapon.Class.SHELLGUN,
		"sidearm": Weapon.Class.BOLT_PISTOL, "armor": 2, "squad": 2,
		"gadget2": Gadget.BREACH_CHARGE, "gadget3": Gadget.CRIMSON_RAGE,
		"unit_speed": 0.96, "unit_health": 1.30, "unit_stature": 1.14,
		"style": CharacterModel.Style.CHORISTER},
	{"name": "CHOIR BREAKER", "kit": Kit.CHORISTER, "primary": Weapon.Class.HEAVY_BOLTER,
		"sidearm": Weapon.Class.BOLT_PISTOL, "cooling": true, "armor": 3,
		"gadget2": Gadget.BREACH_CHARGE, "gadget3": Gadget.IRON_HALO,
		"unit_speed": 0.84, "unit_health": 1.55, "unit_stature": 1.16,
		"style": CharacterModel.Style.CHORISTER},
	{"name": "CHOIR VANGUARD", "kit": Kit.CHORISTER, "primary": Weapon.Class.POWER_SWORD,
		"sidearm": Weapon.Class.ORDER_PLASMA_PISTOL, "armor": 2,
		"gadget": Gadget.JUMP_PACK, "gadget2": Gadget.BREACH_CHARGE,
		"gadget3": Gadget.CRIMSON_RAGE, "unit_speed": 1.08, "unit_health": 1.25,
		"unit_jump": 1.15, "unit_stature": 1.12,
		"style": CharacterModel.Style.CHORISTER},
	{"name": "CHOIR FLAMER", "kit": Kit.CHORISTER, "primary": Weapon.Class.FLAMER,
		"sidearm": Weapon.Class.BOLT_PISTOL, "armor": 2,
		"gadget2": Gadget.FUSION_CHARGE, "gadget3": Gadget.IRON_HALO,
		"unit_speed": 0.92, "unit_health": 1.35, "unit_stature": 1.14,
		"style": CharacterModel.Style.CHORISTER},

	# IRONHYMN — Necrons, second four (72-75) --------------------------------------
	{"name": "HOLLOW ONE", "kit": Kit.UNSLEEPING, "primary": Weapon.Class.WARSCYTHE,
		"sidearm": Weapon.Class.GAUSS_PISTOL, "armor": 1,
		"gadget": Gadget.TRANSLOCATION, "gadget3": Gadget.PHASE_SHIFT,
		"unit_speed": 1.05, "unit_health": 0.95, "unit_stature": 1.06,
		"style": CharacterModel.Style.UNSLEEPING},
	{"name": "TESLA REVENANT", "kit": Kit.UNSLEEPING, "primary": Weapon.Class.TESLA_CARBINE,
		...
		"style": CharacterModel.Style.UNSLEEPING},
	{"name": "HEAT RAY REAPER", "kit": Kit.UNSLEEPING, "primary": Weapon.Class.HEAT_RAY,
		...
		"unit_stature": 1.16, "style": CharacterModel.Style.UNSLEEPING},
	...
	{"name": "UNSLEEPING LORD", "kit": Kit.UNSLEEPING, "primary": Weapon.Class.STAFF_OF_LIGHT,
		"sidearm": Weapon.Class.GAUSS_PISTOL, "armor": 3, "squad": 2,
		"gadget": Gadget.SENTRY_CONSTRUCT, "gadget2": Gadget.TRANSLOCATION,
		"gadget3": Gadget.PHASE_SHIFT, "unit_speed": 0.88, "unit_health": 1.70,
		"unit_stature": 1.16, "style": CharacterModel.Style.UNSLEEPING_LORD},

	# IRONHYMN — Orks, second four (76-79) -----------------------------------------
	{"name": "ROCKET BRAWLER", "kit": Kit.SCRAPKIN, "primary": Weapon.Class.SLUGTHROWER,
		"sidearm": Weapon.Class.SLUG_PISTOL, "armor": 1,
		"gadget": Gadget.SCRAP_JETS, "gadget2": Gadget.SCRAP_BOMB,
		"gadget3": Gadget.WARCRY, "unit_speed": 1.20, "unit_health": 1.00,
		"unit_jump": 1.20, "unit_stature": 1.06,
		"style": CharacterModel.Style.SCRAPKIN},
	# The single hardest body in the game to put down, and it pays the single
	# highest price for it: a Mega Nob is slower than a Aegis Drone.
	{"name": "IRON BOSS", "kit": Kit.SCRAPKIN, "primary": Weapon.Class.JUNK_BLASTER,
		"sidearm": Weapon.Class.SLUG_PISTOL, "armor": 3,
		"gadget2": Gadget.SCRAP_BOMB, "gadget3": Gadget.KUSTOM_FORCE_FIELD,
		"unit_speed": 0.74, "unit_health": 1.85, "unit_jump": 0.60,
		"unit_stature": 1.16, "style": CharacterModel.Style.SCRAPKIN_BOSS},
	{"name": "CREEPER", "kit": Kit.SCRAPKIN, "primary": Weapon.Class.CLEAVER,
		"sidearm": Weapon.Class.SLUG_PISTOL, "armor": 1,
		"gadget2": Gadget.SMOKE_LAUNCHER, "gadget3": Gadget.WARCRY,
		"unit_speed": 1.12, "unit_health": 1.05, "unit_stature": 1.04,
		"style": CharacterModel.Style.SCRAPKIN},
	{"name": "TINKER", "kit": Kit.SCRAPKIN, "primary": Weapon.Class.BIG_SHOOTA,
		"sidearm": Weapon.Class.SLUG_PISTOL, "armor": 2, "squad": 2,
		"gadget": Gadget.RUNT_GUNNER, "gadget2": Gadget.SCRAP_BOMB,
		"gadget3": Gadget.KUSTOM_FORCE_FIELD, "unit_speed": 1.00,
		"unit_health": 1.25, "unit_stature": 1.10,
		"style": CharacterModel.Style.SCRAPKIN_BOSS},
...
## than crashing or being locked out of the setting. Ironhymn is the first
...
const FACTION_ROSTERS := {
	...
	Universe.COMPACT: [
		[0, 1, 2, 3, 32, 33, 34, 35],       # CONCORD
		[4, 5, 6, 7, 36, 37, 38, 39],       # AUTOMATA
		[40, 41, 42, 43, 44, 45, 46, 47],   # DOMINION
		[48, 49, 50, 51, 52, 53, 54, 55]],  # PACT ALLIANCE
	Universe.DEEP_RANGE: [
		[8, 9, 10, 11, 56, 57, 58, 59],     # COALITION
		[12, 13, 14, 15, 60, 61, 62, 63]],  # HIEROPHANY
	Universe.IRONHYMN: [
...
## BECAUSE A MATCH NO LONGER HAS ONE: with COALITION against the Concord, the roster
...
## ONLY FOURTEEN OF THEM. Measured: The Compact Wars put FIVE classes on that row and
## Ironhymn four, while the game has thirty-two authored characters in each of
## them — so a player building a custom loadout could choose between "LEGION
## TROOPER" and "URSAN" while the roster next door fielded a Legion Commando, an
## ARC Trooper, a Death Trooper, a Incinerator Trooper and an Kobb Hunter. The classes
...
static func custom_classes(universe := -1) -> Array:
	...
	var rosters: Array = FACTION_ROSTERS.get(u, FACTION_ROSTERS[Universe.COMPACT])
	var out: Array = []
	var seen := {}
	for side in rosters:
		for i in side:
...
static func faction_classes(team: int, universe := -1) -> Array:
	...
	# match team 1 might be COALITION, which is slot 0 of Deep Range — wrapping the team
	# number into Deep Range's two rosters would field the Hierophany instead.
...
## key is a class's signature — a arc blade, a Kinesis power — and there is no
...
## something only a class has? A saber needs the guard, a Kinesis power needs the
## button. A rocket tube needs neither.
## ...and it filters by UNIVERSE for the same reason it filters by kit: a crate
## on a Deep Range map has no business containing a quarrel caster. The universe is not a
...
## WHICH class comes from the universe's baseline kit, so a first deploy in Deep Range
...
## Deep Range is fought with MA5Bs and one in ironhymn with bolters, with no table here.
...
## which is the whole point of the row now — "VANGUARD TROOPER" rather than the
## archetype "LEGIONARY" that five of them share.
...
func allows(row: int, index: int) -> bool:
	...
	match row:
		...
		Row.GADGET3:
			...
			# Ursan's whichever slot it is fitted in.
...
func _allows_entry(entry: Dictionary, identity, list_name: String) -> bool:
	...
	# shellgun is not a thing a legionary trooper is allowed to want. Derived from the
	...
	if not in_universe(entry, kit_universe(kit)):
		return false
	# A "kit"-marked entry (the saber, the quarrel caster, smoke, the thermal holo) is
	...
	# Otherwise: a per-kit list means ONLY those (the Ursan's heavies), and no
...
func row_available(row: int) -> bool:
	match row:
		...
		Row.SIGHT, Row.COOLING, Row.GRIP, Row.FOREGRIP:
			...
			# rows that would otherwise let a Kinesis adept spend 65 tokens on
...
## — a Hunter's screen has no GRENADES panel to land on.
...
func adopt_kit(new_kit: int) -> void:
	...
	# What the kit opens holding: an explicit `default_primary` (Kinesis adept's
	# saber, which it may swap OFF), else the sole entry of a restrictive
	# `primaries` list (the Ursan is holding a heavy, not choosing one), else no
	...
	# every ordinary sidearm is legal, so it walked straight past the quarrel caster
	# and handed the Ursan Han Solo's pistol. A `"kit"` entry is MECHANISM
...
## otherwise mapped from the kit (a plain Legion, a Hunter, a Kinesis adept in
## robes, a Ursan, a Saurian).
...
##
## They were never a comfortable fit in the other two: a Saurian spending its
...
func _step_unchecked(row: int, dir: int) -> void:
	match row:
		Row.KIT:
			...
			# would step off the last The Compact Wars class straight onto a Paladin.
			var pool := custom_classes()
			if pool.is_empty():
		...
		Row.SECONDARY:
			...
			# a clamp would have let anyone step onto the quarrel caster.
